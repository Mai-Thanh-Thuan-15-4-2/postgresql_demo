...
export class StudentService {
  ...
  async importStudentsFromExcel(buffer: Buffer): Promise<void> {
    //TODO: e optimize nó thêm nha, nhìn thấy chậm ùi, thử với file lớn nha
    if (!buffer) {
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          if (!row['first_name'] || !row['last_name'] || !row['email']) {
            ...
            throw new BadRequestException(
              ERROR_MESSAGES.VALIDATION_EXIT_CODE[3001],
            );
          }
          ...
          if (error.code === '23505') {
            // TODO: E nên define ra 1 enum chung cho các loại lỗi
            throw new BadRequestException(
              ERROR_MESSAGES.DATABASE_EXIT_CODE[4006],
            );
          }
          throw new InternalServerErrorException(
            ERROR_MESSAGES.UNKNOW_EXIT_CODE[9001],
          );
        }
      }
    } catch (error) {
      ...
      throw new InternalServerErrorException(
        ERROR_MESSAGES.SERVER_EXIT_CODE[6001],
      );
    }
  }
}
