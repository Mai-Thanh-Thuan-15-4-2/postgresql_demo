import { Controller, Post, Body, UnauthorizedException, Res, UseFilters, Req } from '@nestjs/common';
...
export class AuthController {
  ...
  async register(
    ...
    const { firstName, lastName, email, username, password, role } = registerDto;
    if (!role) {
      this.logger.error('Role is required for registration.', ERROR_MESSAGES.VALIDATION_EXIT_CODE[3008]);
      throw new UnauthorizedException({ message: ERROR_MESSAGES.VALIDATION_EXIT_CODE[3008] });
    }
    const user = await this.authService.register(firstName, lastName, email, username, password, role);
  ...
  async logout(
    ...
    try {
      ...
      throw new UnauthorizedException({ message: ERROR_MESSAGES.SERVER_EXIT_CODE[6001] });
    }
  }
}